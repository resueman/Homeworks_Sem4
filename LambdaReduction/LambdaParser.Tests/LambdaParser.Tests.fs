...
module LambdaParser.Tests
    ...
    [<Test>]
    [<Category("Should distinguish between abstraction and application")>]
    [<TestCase("let A = \x.x \y.y\nA")>]
    [<TestCase("let A = (\x.x \y.y)\nA")>]
    [<TestCase("let A = \x.x (\y.y)\nA")>]
    [<TestCase("let A = (\x.x (\y.y))\nA")>]
    let ``Should parse applications two abstractions correctly`` (input: string) =
        parseFromString input |> should equal (Some(Abs("x", App(Var "x", Iy))))
    ...
    [<TestCase("let A = ((\x.x) (\y.y))\nA")>]
    [<TestCase("let A = \x.x\nlet B = \y.y\nA B")>]
    let ``Should parse abstraction as part of another abstarction correctly`` (input: string) =
        parseFromString input |> should equal <| Some(App(Ix, Iy))
